...
(defun make-draw-able-object ()
  (let ((%super-class (make-root))
    ...
    (labels ((%set-attr-pointers (cnt l stride offset)
	       (cond ((null l))
		     ;; TODO:
                     (t (gl:vertex-attrib-pointer cnt
						  (car l)
						  :float 
						  :false 
	  ...
	  ((initialize-obj)
	   (lambda (self)
	     (let* ((verts (ask self 'get-vertexes))
		    ...
		    (indices (ask %indices 'get-indices
				  0 (/ verts-length
				       (reduce #'+ vert-attrs))))

		    (inds-length (length indices))
		    (verts-gl-array (gl:alloc-gl-array :float verts-length))
		    (indices-gl-array (gl:alloc-gl-array :unsigned-int
							 inds-length))
		    (index 0))
